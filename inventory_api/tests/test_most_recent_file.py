...
import pytest
from most_recent_file import MostRecentFile
...
def test_chooses_most_recent_file():
    ...
    source_file = os.path.abspath('inventory_api/tests/sample_csv_files/beer_data2018-04-14_16:34:24.131698.csv')
...
def test_empty_file():
    ...
    source_file = os.path.abspath('inventory_api/tests/sample_csv_files/empty.csv')
...
def test_file_single_line():
    temp_dir = tempfile.mkdtemp()
    temp_file = tempfile.NamedTemporaryFile(dir=temp_dir)
    source_file = os.path.abspath('inventory_api/tests/sample_csv_files/categories_only.csv')
    copyfile(source_file, temp_file.name)
    most_recent_file = MostRecentFile(temp_dir)
    test_iterator = iter(most_recent_file)
    with pytest.raises(StopIteration):
